...
import { Filter, ArrowUpDown, Search, ShieldAlert, Package, AlertTriangle } from 'lucide-react';
import API from './api';
...
export default function AlertsPage() {
  ...
  // TIER 1 FIX: Algoritmik hesaplama artık sunucuda dönüyor, frontend sadece sonucu çekiyor
  const fetchProductsAlerts = async () => {
    setLoading(true);
    try {
      const response = await API.get('/alerts');
      setAlerts(response.data);
    } catch (err) {
      console.error("Uyarı listesi yüklenemedi:", err);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-[3rem] border border-gray-50 shadow-sm overflow-hidden flex flex-col">
        <div className="p-6 border-b border-gray-50 flex justify-between items-center bg-gray-50/30">
          ...
          <div className="relative">
            ...
            {isSortMenuOpen && (
              <div className="absolute right-0 top-14 bg-white border border-gray-100 rounded-2xl shadow-xl py-2 w-52 z-40">
        ...
        <div className="overflow-x-auto">
          {loading ? (
            <p className="text-center text-gray-400 py-12 font-bold animate-pulse">Kritik stoklar taranıyor...</p>
          ) : (
              ...
              <tbody className="divide-y divide-gray-50 bg-white">
                {sortedAlerts.map((item) => (
                  <tr key={item.id} className="hover:bg-red-50/10 transition-colors group">
                    ...
                    <td className="px-4 py-4"><span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{item.sku}</span></td>
                    <td className="px-4 py-4"><span className="font-black text-sm text-red-600 bg-red-50 px-3 py-1.5 rounded-lg">{item.stock} units</span></td>
                    <td className="px-4 py-4"><span className="font-bold text-sm text-gray-400">{item.threshold} units</span></td>
