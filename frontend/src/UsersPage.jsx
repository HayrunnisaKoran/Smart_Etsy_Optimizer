...
import { Search, Plus, Edit, Trash2, Shield, Mail, CheckCircle2, XCircle, X, UserCheck } from 'lucide-react';
...
export default function UsersPage() {
  ...
  // 1. Kullanıcıları Firebase'den Canlı Çekme (READ)
  useEffect(() => {
    // Firebase'deki "users" koleksiyonunu dinliyoruz
    ...
    const unsubscribe = onSnapshot(q, (snap) => {
      setUsers(snap.docs.map(doc => {
        ...
        return {
          ...data,
          id: doc.id,
          // İlk kurulumda tarih yoksa varsayılan atama yapıyoruz
          lastLogin: data.lastLogin || 'May 15, 2026 10:30 AM'
        };
      }));
    });
    return () => unsubscribe();
  ...
  // 3. Kullanıcı Ekleme / Güncelleme (CREATE / UPDATE)
  const handleSave = async (e) => {
    ...
    try {
      const userData = {
        ...
        lastLogin: isEditing ? currentUser.lastLogin : new Date().toLocaleString('en-US', { month: 'short', day: 'numeric', year: 'numeric', hour: '2-digit', minute: '2-digit' })
      };
  ...
  return (
    <div className="p-10 space-y-8 animate-in fade-in duration-500">
      ...
      <div className="bg-white rounded-[3rem] border border-gray-50 shadow-sm overflow-hidden flex flex-col min-h-[400px]">
        ...
        {/* KULLANICI LİSTESİ TABLOSU (Görseldeki Sütunlarla Birebir Aynı) */}
                ...
                <tr key={user.id} className="hover:bg-gray-50/50 transition-colors group">
                  ...
                  <td className="px-4 py-4">
                    <span className={`px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest ${
                      user.status === 'Active' ? 'text-green-500 bg-green-50' : 'text-red-400 bg-red-50'
                    }`}>
                      {user.status}
                    </span>
                  </td>
                  <td className="px-4 py-4">
                    <span className="text-xs font-bold text-gray-400">{user.lastLogin}</span>
                  </td>
